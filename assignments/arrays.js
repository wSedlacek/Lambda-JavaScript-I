...
let car33 = inventory.filter(({ id }) => id === 33)[0];
...
console.log(`There are ${oldCars.length} old cars.`);
console.log();

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory. Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = inventory.filter(({ car_make }) => car_make === 'Audi' || car_make === 'BMW');

//console.log(JSON.stringify(BMWAndAudi));
console.log('BMW and Audi cars:');
BMWAndAudi.forEach(car =>
  console.log(`${car.id} -- ${car.car_year} ${car.car_make} ${car.car_model}`)
);
